...
def getServiceManager( cHost="localhost", cPort="8100" ): 
    ...
    if not goServiceManager: 
        if not ctx:
            ...
            goServiceManager = ctx.getServiceManager()
...
# It is cachedXSCRIPTCONTEXT.getComponentContext() in a global variable. 
StarDesktop = None 
def getDesktop(): 
...
def msgBox(title, message, parent=None, message_type="INFOBOX", buttons=1):
    """ Show message in message box. """
    if not parent:
        parent = getDesktop().getCurrentFrame().getContainerWindow()
